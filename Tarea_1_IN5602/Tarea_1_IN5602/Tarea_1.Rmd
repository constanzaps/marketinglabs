...
Sin embargo, este es un efecto muy particular, al que se debe tener en cuenta. La regresión indica también que, en general, los miércoles y jueves el género femenino puntúa 2,609 y 1,781 puntos más bajo en el rating, respectivamente. Es más, en la ciudad de Yangon las mujeres puntúan en promedio un ranking de 1,593 puntos más bajo que el promedio, por lo que se debe estudiar más a fondo lo que sucede 

Este efecto es dsitinto en la ciudad de Yangon, en donde las mujeres en esta ciudad califican su satisfacción con un rating 1,593 puntos más bajo en promedio. El efecto de el género femenino en esta ciudad es particularmente más fuerte los días miércoles, en que muestran un rating 2,609 puntos más bajo.

Pero este efecto negativo del género femenino los miércoles no es particular de Yangon. Esto ocurre los miércoles y jueves independientemente de las ciudades, siendo el ranking 2,609 puntos más bajo los miércoles y 1,781 puntos más bajo los jueves.

En una segunda regresión se propone un modelo de regresión lineal con interacción triple entre las variables Gender, Total y Weekday. En primer lugar, se presume que los clientes pueden tener distinto ánimo dependiendo del día de la semana en que se encuentren, lo que lleva a pensar a que la variable Weekday afecte de manera indirecta en el rating del cliente. Por otro lado, las personas pueden llevarse una sorpresa al revisar el valor total de la compra en su boleta, y esto afectará en el ranking otorgado por el consumidor. Finalmente, el EDA indica que interacciones de variables con el género del consumidor generan diferencias observables en los gráficos, por lo que es una variable interesante de estudio.

```{r P2_a.2}

#lm2 <- lm(Rating ~ Product.line*dia, data = ventas)
#lm2 <- lm(Rating ~ Gender*Payment*dia , data=ventas) #regresión lineal 2 con datos
#lm2 <- lm(Rating ~ Product.line*Weekday*Payment, data=ventas)

lm2 <- lm(Rating ~ Gender*log(Total)*Weekday, data=ventas) #regresión lineal 2 con datos
...
interaccion_lm2 <- model.matrix(~Gender*Weekday*log(Total), data=ventas)
...
ventas_sin_rating <- subset(ventas, select = -c(Rating))

LassoModel <- glmnet(ventas_sin_rating, ventas$Rating, alpha=1, lambda=0.03)
coef(LassoModel)
...
#Creamos las variables para cada tienda:
...
ventas_train_A = ventas[ventas$Branch=="A",]  %>% sample_frac(0.7) #tomamos un 70% de las compras en sucursal A
ventas_train_B = ventas[ventas$Branch=="B",]  %>% sample_frac(0.7) #tomamos un 70% de las compras en sucursal B
ventas_train_C = ventas[ventas$Branch=="C",]  %>% sample_frac(0.7) #tomamos un 70% de las compras en sucursal C

ventas_train_A_todo = ventas_train_A %>%
...
```{r}
x <- seq(0,ventas_train_A_todo$m,by = 1)
y <- dbinom(x, ventas_train_A_todo$m, ventas_train_A_todo$TR)
plot(x,y)
```
