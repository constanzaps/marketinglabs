...
Lo primero que se puede realizar es un histograma de las variables numéricas, en estas podemos incluir las variables Gender y Customer.type como binarias para tener una leve idea de su relación con la variable *Rating*:
```{r}
ventas$Gender[ventas$Gender=="Female"]<-1
ventas$Gender[ventas$Gender=="Male"]<-0
ventas$Gender<-as.numeric(ventas$Gender)

ventas$Customer.type[ventas$Customer.type=="Member"]<-1
ventas$Customer.type[ventas$Customer.type=="Normal"]<-0
ventas$Customer.type<-as.numeric(ventas$Customer.type)
ventas$hora <- as.POSIXct(ventas$Time, format="%H:%M")

nums <- ventas[ , unlist(lapply(ventas, is.numeric))] #variables numéricas
multi.hist(nums) #Histograma múltiple
```
Se puede observar que *gross.margin.percentaje* parece ser una variable constante, lo cual se confirma al tener una desviación estandar igual a 0. Con esto se puede omitir la variable en el análisis de regresiones ya que servirá para predecir ninguna otra variable.
...
```
Donde se observa que *Rating* tiene correlación 0 con todas las demás variables, y varias variables tienen correlación 1, lo cual se tendrá en cuenta al correr cual modelo.
...
ggarrange(bp, bp1, bp2, bp3, bp4, bp5, 
          labels = c("Product.line", "City", "Payment","Gender","Branch","Customer.type"),font.label = list(size = 10, color = "black"),
          ncol = 2, nrow = 3)
...
ventas$day <- weekdays(ventas$Date)
ventas$day = factor(ventas$day, levels=c('lunes','martes','miércoles','jueves', 'viernes', 'sábado', 'domingo'))
