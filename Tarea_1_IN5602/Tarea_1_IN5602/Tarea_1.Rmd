...
train.randomf <- train(Rating ~ City + log(Total) + Time +City*Gender*Weekday,
...
train.knn <- train(Rating ~ City + log(Total) + Time +City*Gender*Weekday, 
...
En general se ve que los errores  son menores para los modelos de Machine Learning, lo cual concuerda con la intuición de que estos tienen mejor capacidad predictiva para modelos más complejos, y servirían mejor para el objetivo de predecir el rating.
...
En la P4 no solo se debe comparar la predicción, también el ajuste y la forma de los modelos que están utilizando. También se utilizan métricas de evaluación en este caso. Con forma me refiero, por ejemplo, a que regresión lineal asume una forma a lineal en las relaciones, MARS asume no linealidades con puntos de corte, y así. Finalmente con la comparación en predicción, ajuste y la reflexión sobre sus modelos, deberían decidir cual es el mas completo para usar de manera de determinar la dependiente (ratings).

Para comparar su capacidad predicctiva recordamos que el modelo Random Forest es el que presenta el menor error para las métricas RMSE (1.706) y MAE (1.451), sin embargo el modelo de regresión lineal es el que tiene mayor R^2, es decir, explica mejor la variabilidad de los datos, sin embargo el R^2 del random forest le sigue muy de cerca, y se puede observar en los boxplot de la pregunta anterior que el random forest es el que tiene menor rando de errores, por lo cual el mejor modelo predictivo para este caso sería Random Forest.
...
Además se debe considerar la forma en que trabajan los modelos. El modelo KNN se basa en encontrar grupos de objetos similares, y luego usa el valor de respuesta media de k observaciones como el resultado previsto. El modelo de Random Forest es un conjunto de varios árboles de decición que van agregando grupos para predecir cual será el objetivo, esta agrupación hace que este tenga menos variabilidad al clasificar, y suele entregar mejores predicciones. Y el modelo de regresión lineal busca minimizar la distancia entre la regresión y los datos buscando un mejor ajuste, sin embargo al ver las relación lineal de las variables con "y", y no es capaz de encontrar relaciones más complejas. En esto se observa que el modelo más adecuado vuelve a ser el Random Forest, pues se ha verificado la importancia de la interacción entre variables para comprender el rating.
...
```{r, fig.width=15, fig.height=45, include=FALSE}
...
g <- vector("list", length = length(genero)*length(producto)*length(branch))
#g = list()
...
for (tienda in branch){
  for (m_h in genero){
    for (pr in producto){
        ...
        x_train <- seq(0,train_prob$m,by = 1)
        y_train <- dbinom(x_train, train_prob$m, train_prob$TR)
        x_train_norm <- x_train*100/train_prob$m
        
        x_test <- seq(0,test_prob$m,by = 1)
        y_test <- dbinom(x_test, test_prob$m, test_prob$TR)
        x_test_norm <- x_test*100/test_prob$m
        
        message(i)
        g[[i]] <- local({
          i <- i
          plot <- ggplot()+
            geom_line(aes(x=x_train_norm,y=y_train, colour='red'),  size=2)+
            geom_line(aes(x=x_test_norm ,y=y_test , colour='blue'), size=2)+
            theme(plot.title = element_text(hjust = 0.5))+
            scale_color_discrete(name='Sets', labels = c("Train", "Test"))+
            labs(x='Proporcion de clientes satisfechos (%)',
               y='Probabilidad',
