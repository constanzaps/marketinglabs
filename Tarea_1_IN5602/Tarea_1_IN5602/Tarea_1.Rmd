...
```{r P2_a}
train.lm <- train(form = Rating ~ Branch + Customer.type + Gender + Product.line +
                     Payment + Quantity + Total + Time + Weekday + 
                    Product.line*Branch + Weekday*Product.line + Gender*Payment +
                    Gender*Product.line ,#Fórmula
  data = train, #Datos
  method = "lm", #Algoritmo 
...
xtrain <- subset(train, select = -c(gross.margin.percentage, Rating, City, Tax.5., cogs, gross.income))
LassoModel <- glmnet(xtrain, train$Rating, alpha=1, lambda=0.03)
coef(LassoModel)
```
