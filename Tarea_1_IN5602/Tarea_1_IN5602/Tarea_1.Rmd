...
Las variables categoricas tienen una distribución similiar entre sus distintos valores, por lo que no hay un desbalance entre clases. Sobre las variables númericas, a simple vista los percentiles parecen indicar que no outliers de gran magnitud en las variables. Para confirmar este supuesto se realiza un histograma para cada variable numérica:
...
Se puede observar que *gross.margin.percentaje* parece ser una variable constante, lo cual se confirma al tener una desviación estandar igual a 0. Con esto se puede omitir la variable en el análisis ya que no servirá para predecir ninguna otra variable.
...
Con ello se puede observar que la mayoría de estas categorías están repartidas con igual porcentaje dentro de la base de datos, siendo la excepción las fechas, donde existen algunos días más frecuentes que otros. Otro dato interesante es que cada marca se encuentra en una ciudad distinta, por lo cual bastará con estudiar solo una de las dos variables.

Se puede ver en más detalle las diferencias en días en la siguiente tabla, que explica que la menor frecuencia que tiene un día específico del mes es 14 y la máxima 44.
```{r}
...
g1 <- ggplot(ventas) +
  ...
  geom_point(size=1, alpha=0.4, colour = "darkblue")+
  geom_smooth(se=FALSE, colour='red') + #Agregamos un ajuste no lineal sobre los puntos.
  scale_y_log10() +
  xlab("Satisfacción")+
  ylab("log(Total)")+
  ggtitle("log(Total) en relación a la satisfacción")
 ...
 g2 <- ggplot(ventas) +
  ...
  xlab("Cantidad de productos comprados \n según Satisfacción") 
...
Luego se puede realizar un análisis de las variables categóricas, donde se observa que sí existen leves diferencias en las distribuciones de rating, notandose un poco más en las variables  *Product.line* y según género.
...
g2 <- ggplot(ventas, aes(x=Rating, fill=factor(Product.line))) + geom_density(alpha=0.2)+ facet_grid(City~Product.line)+theme(legend.position = "none")
...
g3 <- ggplot(ventas, aes(x=Rating, fill=factor(Product.line))) + geom_density(alpha=0.4)+
  facet_wrap(Customer.type~Product.line, ncol = 3)
g3
```
...
#train.lm1 <- train(form = Rating ~ City + Gender + Product.line + Payment
#                   + Product.line*Payment + City*Gender,#Fórmula
#                   data = train, #Datos
#                   method = "lm", #Algoritmo 
#                   trControl = trainControl(method = "cv", number = 10) #Method = cross validation, number=10 (k-fold) 
#)
lm1 <- lm(Rating ~ City + Gender + Product.line + Payment + Product.line*Payment
          + City*Product.line, data=ventas) #regresión lineal 1 con datos

#test.lm1  <- predict(train.lm1 , newdata=test) #Vector de datos predichos. Recibe una base de datos (newdata) y un modelo entrenado (train.lm)
#error.lm <- test$Rating-test.lm #Calcular los errores de predicción (dato real - dato estimado)
#error.lm

#Mape_lm1 <- MAPE(test$Rating, test.lm1)
#RMse_lm1 <- RMSE(test$Rating, test.lm1)
...
#train.lm2 <- train(form = Rating ~ City + Product.line + Payment + Time
#                   + Product.line*Total + Product.line*Weekday
#                   ,#Fórmula
#                   data = train, #Datos
#                  method = "lm", #Algoritmo 
#                 trControl = trainControl(method = "cv", number = 10) #Method = cross validation, number=10 (k-fold) 
...
lm2 <- lm(Rating ~ City + Product.line + Payment + Time + Product.line*Total
          + Product.line*Weekday, data=ventas) #regresión lineal 2 con datos
#test.lm2  <- predict(train.lm1 , newdata=test) #Vector de datos predichos. Recibe una base de datos (newdata)

#Mape_lm2 <- MAPE(test$Rating, test.lm2)
#RMse_lm2 <- RMSE(test$Rating, test.lm2)
...
En base a lo observado en la P2, se escogen las variables City, Total, Product.line, Payment, Gender. Además se escogen las interacciones que dieron resultados más interesantes en la P1, es decir City y Gender, Payment y Gender, y finalmente Product line y Total que dieron resultados significactivos en la P2.
...
train.randomf <- train(Rating ~ City + Total + Time + Gender + Product.line 
                   + Payment +Customer.type+ Gender*Payment + City*Gender+Product.line*Total,
...
train.knn <- train(Rating ~ City + Total + Time + Gender + Product.line 
                   + Payment +Customer.type+ Gender*Payment + City*Gender+Product.line*Total, #+ Product.line*Payment si agreggo esto son 13
...
train.lm <- train(Rating ~ City + Total + Time + Gender + Product.line 
                   + Payment +Customer.type+ Gender*Payment + City*Gender+Product.line*Total, #+ Product.line*Payment si agreggo esto son 13
...
Para comparar la capacidad de estos modelos frente a la regresión lineal debemos recordar que estos modelos son muy útiles cuando el foco del problema está en el pronóticos de relaciones complejas y no lineales con grandes cantidades de datos, en cambio las regresiones lineales son muy útiles para entender la relación de las variables con los componentes de la regresión. 
