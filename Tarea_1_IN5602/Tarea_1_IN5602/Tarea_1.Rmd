...
g2 <- ggplot(ventas, aes(y=Rating, x=Product.line, fill = Product.line))+
  geom_boxplot(alpha=0.4)+
  theme(plot.title = element_text(hjust = 0.5))+
  labs(y='Satisfacción', x='Día', title='Satisfacción con respecto al tipo de producto')

g3 <- ggplot(ventas, aes(y=Rating, x=Branch, fill = Branch))+
  geom_boxplot(alpha=0.4)+
  theme(plot.title = element_text(hjust = 0.5))+
  labs(y='Satisfacción', x='Día', title='Satisfacción con respecto a la tienda')

fig <- ggarrange(g3, g1, g2, nrow=3)
fig
```
A simple vista no existen relaciones muy marcadas entre las comparaciones. Según los gráficos, los días con menor satisfacción
