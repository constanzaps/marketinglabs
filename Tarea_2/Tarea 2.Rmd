...
anime <- read.csv(paste0(path,"anime.csv"))
anime <- anime[-c(1)]
anime$Expertos <-as.numeric(anime$Expertos)
...
Si bien al estar la base en formato long pierde sentido ver las correlaciones de la mayoría de las variables, es interesante observar como se correlaciona "Elección" con las demás. En particular se puede observar que está correlacionada de forma positiva con "RatingTV", y de forma negativa con la duración del capítulo, el número de escenas de combate y curiosamente con la calidad de la animación.
...
calidad_anime <- ggplot(anime, aes(x =factor(Animes) ,y=CalidadAnimacion,color=Animes)) + geom_boxplot()+ theme(legend.position="none")+ ggtitle('Calidad de animación por anime')
rating_anime<- ggplot(anime, aes(x =factor(Animes) ,y=RatingTV,color=Animes)) + geom_boxplot()+ theme(legend.position="none")+ ggtitle('Rating TV por anime')

ggarrange(calidad_anime,rating_anime, nrow=1, ncol=2)
```

```{r, fig.width=9, fig.height=3, dpi=200, echo=FALSE}
comba.anim <- ggplot(anime, aes(x =factor(Animes) ,y=NEscenasCombate,color=Animes)) + geom_boxplot()+ theme(legend.position="none")+ ggtitle('Cantidad de escenas de combate por anime')#+
emocio.anim <-ggplot(anime, aes(x =factor(Animes) ,y=NescenasEmocionantes,color=Animes)) + geom_boxplot()+ theme(legend.position="none")+ ggtitle('Cantidad de escenas emocionantes por anime')
...
anime1 <- subset(anime1, select=-c(Eleccion,Animes))
anime1
summary(anime1)
```
...
cate_expertos <- unique(subset(anime1,select=c(Expertos, Mujer, Edad)))
 ...
 DuracionCapituloMin.hist <- ggplot(anime1, aes(x=DuracionCapituloMin, fill=..count..))+
  geom_histogram(bins= 5)+
  labs(x = 'Duración (min)', y = 'Count', title = 'Duración de capítulos: Histograma ')+
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5))
 
ggarrange(Elegidos.hist,ggarrange(Mujer.hist,Edad.hist, nrow=2, ncol=1),DuracionCapituloMin.hist ,ncol = 3)
```
...
tabla <- as.data.frame(table(anime1$Capitulos,anime1$Elegido))
tabla
...
Esto lo confirmamos en la siguiente matriz de correlación, donde además se puede observar que existe una correlación negativa entre la calidad de la animación y el rating que tiene en televisión.
```{r, fig.width=6, fig.height=3, dpi=200, echo=FALSE}
...
```{r P2_l}
anime_logit <- anime
anime_logit$Eleg[anime_logit$Eleccion== 0] <- FALSE
anime_logit$Eleg[anime_logit$Eleccion== 1] <- TRUE
anime_logit <- subset(anime_logit, select=-c(Eleccion))
logit_data <- mlogit.data(anime_logit, shape = "long", choice ="Eleg",id.var = "Expertos")
#corremos el modelo



colnames(anime_logit)
modelo1<-mlogit(Eleg ~ NEscenasCombate + NescenasEmocionantes + RatingTV + CalidadAnimacion + DuracionCapituloMin | Edad + Mujer | Scoremyanimelist + DuracionCapituloMin, data = logit_data)
```
