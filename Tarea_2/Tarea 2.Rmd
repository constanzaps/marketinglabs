...
nums <- subset(anime[ ,unlist(lapply(anime, is.numeric))],
               select=-c(CantCaps,Edad,Expertos,Scoremyanimelist,Mujer,Capitulos)) #Animes,
...
ggplot(anime1, aes(x=Edad, fill=factor(Elegido))) + geom_histogram(bins = 20)+
  facet_wrap(Mujer~Elegido,nrow=2)+ scale_fill_manual(values = c("#56b1f7", "#4793cf", 
...
anime2 <- unique(subset(anime,select=-c(X,Expertos,Edad,Mujer,Eleccion,Elegido)))
...
nums <- subset(anime2[ ,unlist(lapply(anime2, is.numeric))],
               select=-c(Capitulos,CantCaps)) #Animes,
ggcorr(cor(nums), hjust = 1, size = 4,low = "#56b1f7", mid = "white", high = "#132b43",layout.exp = 3,label = TRUE, label_size = 4, label_round = 2)
```
