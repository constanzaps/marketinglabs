...
dur <- ggplot(anime2, aes(x=DuracionCapituloMin, fill=..count..))+
  geom_histogram(bins= 5)+
  labs(x = 'Duración (min)', y = 'Count', title = 'Duración de capítulos: Histograma ')+
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5))
anime2
score<-   ggplot(anime2, aes(x =factor(Animes) ,y=Scoremyanimelist))+
  geom_point(bins= 4)+
  labs(x = 'Duración (min)', y = 'Count', title = 'Duración de capítulos: Histograma ')+
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5))
ggarrange(dur,ggarrange(Mujer.hist,Edad.hist, nrow=2, ncol=1) ,ncol = 2)
```
...
modelo1<-mlogit(Eleccion ~ NEscenasCombate + NescenasEmocionantes + RatingTV| Edad + Mujer |0, data = myf)
summary(modelo1)
```
