...
Escenas emocionantes, calidad y inter 2 

Al ver el modelo con correlación, lo primero que se puede destacar es que el intercepto 2, y la calidad de la animación dejan de ser significativas, mientras que las escenas emocionantes pierden significancia.
...
Además, respecto al ajuste de los datos al modelo, entre los 4 modelos que se consideraron, este es el que tiene el mejor ajuste, llegando el R^2 a un valor de 0.015, que sigue siendo bastante bajo:c.
...
```{r P3_pr1}
modelo_probit1<-mlogit(Eleccion ~  NEscenasCombate + NescenasEmocionantes + CalidadAnimacion | 1 | 0, data = myftrain, probit = TRUE, print.level=1) 

summary(modelo_probit1)
```

*R:* Interprete coeficientes
