...
- Mlogit con interacción
```{r}
colnames(anime)
modelo1<-mlogit(Eleccion ~ NEscenasCombate + NescenasEmocionantes + CalidadAnimacion+ RatingTV+NescenasEmocionantes*RatingTV| Edad + Mujer | 0, data = myftrain, print.level=1) 
summary(modelo1)
```

- Mlogit sin interacción
```{r}
modelo2<-mlogit(Eleccion ~ NEscenasCombate + NescenasEmocionantes + CalidadAnimacion+ RatingTV | Edad + Mujer | 0, data = myftrain) #La wea no cambia nada si quitar 
summary(modelo2)
```
Quitar:
- DuracionCapituloMin
...
```{r}
modelo3<-mlogit(Eleccion ~ NEscenasCombate + NescenasEmocionantes + CalidadAnimacion+ RatingTV + DuracionCapituloMin| Edad + Mujer | 0, data = myftrain) #La wea no cambia nada si quitar 
summary(modelo3)
```

```{r}
modelo5<-mlogit(Eleccion ~ NEscenasCombate + NescenasEmocionantes + CalidadAnimacion+ DuracionCapituloMin | log(Edad) + Mujer | 0, data = myftrain) #La wea no cambia nada si quitar 
summary(modelo5)
```
...
```{r}
modelo6<-mlogit(Eleccion ~ NEscenasCombate + NescenasEmocionantes + CalidadAnimacion| log(Edad) + Mujer | 0, data = myftrain) #La wea no cambia nada si quitar 
summary(modelo6)
```

```{r}
modelo7<-mlogit(Eleccion ~ NEscenasCombate + NescenasEmocionantes + CalidadAnimacion + RatingTV| Edad + Mujer | 0, data = myftrain) #La wea no cambia nada si quitar 
summary(modelo7)
```
...
model_mxlogit.v1<-mlogit(Eleccion ~ NEscenasCombate + NescenasEmocionantes + CalidadAnimacion
                        +RatingTV| Edad + Mujer | 0, data = myftrain,
                        rpar=c("(Intercept):2"="n", "(Intercept):3"="n", "(Intercept):4"="n",
                        NEscenasCombate="n",NescenasEmocionantes="n",CalidadAnimacion="n",
                        RatingTV="n"), correlation=FALSE, print.level = 1,  R = 100)
...
model_mxlogit.v2<-mlogit(Eleccion ~ NEscenasCombate + NescenasEmocionantes + CalidadAnimacion
                        +RatingTV| Edad + Mujer | 0, data = myftrain,
                        rpar=c("(Intercept):2"="n", "(Intercept):3"="n", "(Intercept):4"="n",
                        NEscenasCombate="n",NescenasEmocionantes="n",CalidadAnimacion="n",
                        RatingTV="n"), correlation=TRUE, print.level = 1,  R = 100)
...
# #probit
# library(lmtest)
probit<- glm(Eleccion ~  NEscenasCombate + NescenasEmocionantes + CalidadAnimacion+ RatingTV,data=anime, family=binomial(link="probit"))
...
Probit con mlogit
```{r}
modelo_probit<-mlogit(Eleccion ~  NescenasEmocionantes + CalidadAnimacion+ RatingTV| 1 | 0, data = myftrain, probit = TRUE, print.level=1) 
...
Probit con mlogit
```{r}
modelo_probit2<-mlogit(Eleccion ~  NescenasEmocionantes + RatingTV| 1 | 0, data = myftrain, probit = TRUE, print.level=1) 
...
```{r P4.1}
#ML 1: SVM
train.svm <- train(Eleccion ~ NEscenasCombate + NescenasEmocionantes + CalidadAnimacion
                        +RatingTV| Edad + Mujer | 0, 
                   data=AnimeWideTrain, method="svmLinear2", 
...
test.svm <- predict(train.svm, newdata=AnimeWideTest)
CM.svm   <- confusionMatrix(test.svm, AnimeWideTest$Eleccion)
```

```{r P4.2}
#ML2: KNN
train.knn <- train(Eleccion ~ NEscenasCombate + NescenasEmocionantes + CalidadAnimacion
                        +RatingTV| Edad + Mujer | 0, 
                   data=AnimeWideTrain, method="knn",
                   trControl  = trainControl("cv", number=5, classProbs = TRUE, summaryFunction = twoClassSummary),
                   preProcess = c("center","scale"),
                   tuneLength = 3,
                   metric = "ROC")
ggplot(train.knn)
...
test.knn <- predict(train.knn, newdata=AnimeWideTest)
CM.knn   <-confusionMatrix(test.knn, AnimeWideTest$Eleccion)
```

```{r}
#Logit
train.logit <- train(Eleccion ~ NEscenasCombate + NescenasEmocionantes + CalidadAnimacion
                        +RatingTV| Edad + Mujer | 0, 
