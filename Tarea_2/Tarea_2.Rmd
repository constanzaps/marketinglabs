...
```{r Logit}
modelo1Logit<-mlogit(Eleccion ~ NEscenasCombate + NescenasEmocionantes + CalidadAnimacion+ RatingTV + DuracionCapituloMin | Edad + Mujer | 0, data = myftrain, print.level=1) 
...
for (i in c(1:12)) {
  ...
  prob[i] <- exp(UtilidadNaruto[i])/(exp(UtilidadNaruto[i]) + exp(UtilidadHunter[i]) + exp(UtilidadDragon[i]) + 
                                       exp(UtilidadHero[i]))
}
